    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {mockChannels.map(channel => (
          <div key={channel.id} className="relative w-full aspect-[9/16] min-w-[180px] min-h-[280px] rounded-lg overflow-hidden bg-base-200 group shadow-md flex flex-col justify-between">
            <div className="p-4 flex flex-col items-center">
              ...
              <span className="text-lg font-semibold text-center mb-1">
                {channel.name}
              </span>
            ...
            <div className="p-4 bg-gradient-to-t from-black/80 to-transparent text-white flex flex-col gap-1">
              <span className="text-xs">Subscribers: <span className="font-bold">{channel.subscribers}</span></span>
              <span className="text-xs">Market Cap: <span className="font-bold">{channel.marketCap}</span></span>
            </div>
          </div>
        ))}
      </div>
