...
import { getCoinsNew } from "~~/lib/api";
// If the file does not exist, create '/workspaces/zorapp/packages/nextjs/lib/api.ts' and export getCoinsNew from there.
// Update the import path if the file exists elsewhere, for example:
import { usePostStore } from "~~/stores/postStore";
// If the file does not exist, create '/workspaces/zorapp/packages/nextjs/stores/postStore.ts' and export usePostStore from there.
...
export default function ExplorePage() {
  ...
  const filteredCoins = useMemo(() => {
    switch(activeFilter) {
      ...
      case 'on-chain':
        if (isLoading) return [];
        if (postStoreError) return [];
        if (!contractPosts.length) return [];
  ...
  }, [coins, activeFilter, address, contractPosts, isLoading, postStoreError]);
                        ...
                        <div className="absolute bottom-0 inset-x-0 bg-base-300 p-2 flex flex-col items-center gap-1 rounded-b-xl">
                          <div className="flex justify-around w-full text-xs font-small">
                            <div className="flex flex-col items-center">
                              <span>MCAP</span>
                              <span className="font-bold">{parseFloat(safeFormatEther(c.marketCap)).toFixed(2)}k</span>
                            </div>
                            <div className="flex flex-col items-center">
                              <span>24h</span>
                              <span className="font-bold">{parseFloat(safeFormatEther(c.volume24h)).toFixed(2)}</span>
                            </div>
                            <div className="flex flex-col items-center">
                              <span>24h Δ</span>
